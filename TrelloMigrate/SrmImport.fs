...
let private importProfiles (profiles : Map<string, ProfileWithHandles>) = profiles |> Map.map (fun _ profile -> importProfileAndUpdateIds profile)     
...
let private importSessions sessions = sessions |> Array.iter Session.post
